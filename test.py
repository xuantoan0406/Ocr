	...
	def predict_text(self, pathImg):
		...
		for box in fullBox:
			if box:
				...
				fullLine.append(lineImage)
			else:
				iTab.append(i)
			i += 1
		fullTextPredict = self.detector.predict_batch(fullLine)
		print(fullTextPredict, iTab)
		# 	if box:
		# 		textPredict = self.detector.predict(image.crop((box[0], box[1], box[2], box[3])), return_prob=False)
		# 	if textPredict[1] > 0.6:
		# 		if fullTextPredict and len(fullTextPredict[-1]) > 3 and fullTextPredict[-1].endswith('\t'):
		# 			fullTextPredict[-1] = fullTextPredict[-1] + textPredict[0]
		# 		else:
		# 			if textPredict[0]:
		# 				fullTextPredict.append(textPredict[0])
		# else:
		# 	if fullTextPredict:
		# 		fullTextPredict[-1] = fullTextPredict[-1] + 3 * '\t'
		# 	else:
		# 		continue
...
st = time.time()
a = PredictText()
a.predict_text("image/letter_0.png")
print(time.time() - st)